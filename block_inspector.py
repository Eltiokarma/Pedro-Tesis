...
# ════════════════════════════════════════════════════════
#  TOKENS — promovidos a tokens.py (artboard 1a del rediseño)
# ════════════════════════════════════════════════════════
# La paleta TOK, los temas/acentos/densidades, el bus de preferencias y
# su persistencia viven ahora en tokens.py — fuente canónica del sistema
# de diseño.  Este módulo re-exporta los mismos objetos por
# compatibilidad con los imports históricos (`from block_inspector
# import TOK, _PrefsBus, ...`).
...
def __getattr__(name):
    # ROW_PAD / SECT_GAP se re-vinculan en tokens.apply_preferences;
    # delegamos el acceso para que block_inspector.ROW_PAD siga fresco.
    if name in ("ROW_PAD", "SECT_GAP"):
        return getattr(_tokens, name)
    raise AttributeError(f"module 'block_inspector' has no attribute {name!r}")
...
class BlockInspectorPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._content_lay.setSpacing(_tokens.SECT_GAP)
    ...
    def _row(self, label: str, control, info: str = "") -> QFrame:
        ...
        lay.setContentsMargins(0, _tokens.ROW_PAD//2, 0, _tokens.ROW_PAD//2)
        return r
    ...
    def _section_diagnostico(self, b, eq_type) -> QFrame:
        ...
        evidence_specs = [
            ...
            ("Flash — reparto por componente (x/y/K)",
                                       lambda: None,
                                       lambda: _ev.flash_split_table_text(b)),
            ("Separador mecánico",     lambda: _ev.mech_sep_metrics(b),
                                       lambda: _ev.mech_sep_text(b)),
            ("Splitter",               lambda: _ev.splitter_metrics(b, fs),
                                       lambda: _ev.splitter_text(b, fs)),
            ("Tanque",                 lambda: _ev.tank_metrics(b, fs),
                                       lambda: _ev.tank_text(b, fs)),
            ("Secador",                lambda: _ev.dryer_metrics(b, fs),
                                       lambda: _ev.dryer_text(b, fs)),
            ("Cristalizador",          lambda: _ev.crystallizer_metrics(b, fs),
                                       lambda: _ev.crystallizer_text(b, fs)),
            ("Evaporador",             lambda: _ev.evaporator_metrics(b, fs),
                                       lambda: _ev.evaporator_text(b, fs)),
            ("Caldera",                lambda: _ev.boiler_metrics(b, fs),
                                       lambda: _ev.boiler_text(b, fs)),
            ("Válvula",                lambda: _ev.valve_metrics(b, fs),
                                       lambda: _ev.valve_text(b, fs)),
            ("Mixer",                  lambda: _ev.mixer_metrics(b, fs),
                                       lambda: _ev.mixer_text(b, fs)),
