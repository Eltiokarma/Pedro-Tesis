...
def reactor_text(block) -> Optional[str]:
    ...
    try:
        ...
        if cust:
            # C.3 ciclo 4: los dicts custom persisten `origin` — separar
            # las que nacieron del predictor de las escritas a mano.
            n_pred = sum(1 for c in cust if isinstance(c, dict)
                         and c.get("origin") in ("predicted", "auto"))
            det = f" ({n_pred} del predictor)" if n_pred else ""
            lines.append(f"Custom      {len(cust)} reacción(es) "
                         f"ad-hoc{det}")
        if (mode == "stoich"
...
def pump_figure(block, fs):
    ...
    try:
        ...
        _style_fig(fig, ax)
        ax.plot(q * Q_op, H, lw=2.0, color=_tok("spec"), label="H–Q típica")
        ...
        ax2.tick_params(colors=_tok("ink_soft"))
        ax2.set_ylim(0, 100)
        ...
        _legend(ax, h1 + h2, l1 + l2, fontsize=7, loc="lower left")
...
def _style_fig(fig, ax):
    """Estilo base theme-aware de las figuras de evidencia (mismo sistema
    que econ_figures): fondos, spines, ticks y labels desde TOK."""
    ink = _tok("ink"); soft = _tok("ink_soft")
    fig.patch.set_facecolor(_tok("bg_elev"))
    ax.set_facecolor(_tok("bg_elev"))
    for sp in ax.spines.values():
        sp.set_color(_tok("line_strong"))
...
def _legend(ax, *args, **kw):
    """ax.legend(...) con texto/fondo tokenizados (legible en dark)."""
    leg = ax.legend(*args, **kw)
    if leg is not None:
        for t in leg.get_texts():
            t.set_color(_tok("ink"))
        leg.get_frame().set_facecolor(_tok("bg_elev"))
        leg.get_frame().set_edgecolor(_tok("line"))
    return leg
...
def mccabe_figure(block, fs):
    ...
    try:
        ...
        _style_fig(fig, ax)
        xs, ys = d["equilibrium"]
        ax.plot([0, 1], [0, 1], color=_tok("ink_ghost"), lw=0.8)
        ax.plot(xs, ys, color=_tok("spec"), lw=1.4)
        if not d.get("feasible", True):
...
def profile_figure(block, fs):
    ...
    try:
        ...
        ax.plot(xs, Ts, color=_tok("danger"), marker="o", ms=3, lw=1.0)
        ax.set_xlabel("etapa  (1 = tope)", fontsize=8)
        ax.set_ylabel("T (°C)", color=_tok("danger"), fontsize=8)
        ax.tick_params(axis="y", labelcolor=_tok("danger"), labelsize=7)
        ax.tick_params(axis="x", labelsize=7)
        ax2 = ax.twinx()
        ax2.plot(xs, xL, color=_tok("spec"), marker="s", ms=3, lw=1.0)
        ax2.set_ylabel(f"x ({p['LK']})", color=_tok("spec"), fontsize=8)
        ax2.tick_params(axis="y", labelcolor=_tok("spec"), labelsize=7)
        ax2.set_ylim(0, 1)
...
def reactor_figure(block, fs):
    ...
    try:
        ...
        if mode == "pfr":
            ...
            _legend(ax, fontsize=6, loc="best", ncol=2)
    ...
    try:
        ...
        if approach is not None:
            ...
            ax.annotate(f"ΔT_min = {approach:.1f} °C",
                        ...
                        fontsize=7, ha="center", color=_tok("ink_mute"),
...
def compressor_figure(block, fs):
    ...
    try:
        ...
        ax.plot(rs, [T_in_K * r ** m_s - 273.15 for r in rs],
                color=_tok("spec"), lw=1.4,
                label=f"isentrópico (W={cs['W_isen_kW']:.1f} kW)")
        ax.plot(rs, [T_in_K * r ** m_r - 273.15 for r in rs],
                color=_tok("orange"), lw=1.4,
                label=f"real η={eta:.2f} (W={cs['W_act_kW']:.1f} kW)")
        ax.plot([ratio], [T_isen_K - 273.15], "o", color=_tok("spec"), ms=5)
        ax.plot([ratio], [T_real_K - 273.15], "o", color=_tok("orange"), ms=5)
        ...
        ax.tick_params(labelsize=7)
        ax.grid(True, alpha=0.3, linestyle="--")
        _legend(ax, fontsize=6.5, loc="best")
...
def equilibrium_figure(block, fs):
    ...
    try:
        ...
        ax = fig.add_subplot(111)
        _style_fig(fig, ax)
        ax.plot(Ts, Xs, color=_tok("spec"), lw=1.5,
        ...
        ax.tick_params(labelsize=7)
        ax.grid(True, alpha=0.3, linestyle="--")
        _legend(ax, fontsize=6, loc="best")
